...
import { LoginResponseDto } from '@/auth/application/ports/output/dtos/login-response.dto';
...
export class AuthController {
  ...
  async login(@Body() loginDto: LoginDto, @Req() request: Request): Promise<InternalLoginDataDto> {
    ...
    const applicationDto = AuthMapper.presentationToApplicationLogin(loginDto);
    return this.loginUseCase.execute(applicationDto, ip || '');
  }
