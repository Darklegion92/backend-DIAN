...
import {
  ...
  FacturaImpuestosDto,
  MedioDePagoDto,
} from '../../../presentation/dtos/request/factura-general.dto';
...
export class InvoiceTransformerService
  implements DocumentTransformer<InvoiceRequestDto>
{
  constructor(
  ...
  async transform(
    ...
    const resolutionNumber = await this.resolutionService.getResolutionNumber(
      prefix,
      companyId,
    );
    ...
    const { taxes, with_holding_taxes } =
      await this.generateDataService.generateTaxtotals(
        factura.impuestosGenerales?.FacturaImpuestos || [],
        this.catalogService,
      );
    ...
    const customer = await this.generateDataService.generateCustomer(
      factura.cliente,
      this.catalogService,
      token,
    );
    const invoiceLines = await this.generateInvoiceLines(
      factura.detalleDeFactura.FacturaDetalle,
      taxes,
    );

    const paymentForm = await this.generatePaymentForms(
      factura.mediosDePago.MediosDePago,
    );

    //TODO: validamos si el iva de 0% que manda

    const totalTaxes: number = taxes.reduce(
      (acc, tax) => acc + (tax.percent === 0 ? tax.taxable_amount : 0),
      0,
    );
    const totalTaxesDetail: number = invoiceLines.reduce(
      (acc, line) =>
        acc +
        line.tax_totals.reduce(
          (acc, tax) =>
            acc +
            (tax.percent === 0 && tax.tax_id === 1 ? tax.taxable_amount : 0),
          0,
        ),
      0,
    );
    ...
    if (totalTaxes !== totalTaxesDetail) {
      ...
      const existTaxZero = taxes.some(
        (tax) => tax.percent === 0 && tax.tax_id === 1,
      );
      if (!existTaxZero) {
        for (const line of invoiceLines) {
          if (
            line.tax_totals.some((tax) => tax.percent === 0 && tax.tax_id === 1)
          ) {
            if (line.tax_totals.length > 1) {
              line.tax_totals = line.tax_totals.filter(
                (tax) => tax.tax_id !== 1,
              );
            } else {
              legalMonetaryTotals.tax_exclusive_amount =
                legalMonetaryTotals.tax_exclusive_amount -
                line.tax_totals[0].taxable_amount;
              delete line.tax_totals;
            }
          }
        }
      } else {
        const index = invoiceLines.findIndex((line) =>
          line.tax_totals.some(
            (tax) =>
              tax.percent === 0 &&
              tax.tax_id === 1 &&
              tax.taxable_amount === diff,
          ),
        );
        if (index !== -1) {
          if (invoiceLines[index].tax_totals.length > 1) {
            invoiceLines[index].tax_totals = invoiceLines[
              index
            ].tax_totals.filter((tax) => tax.tax_id !== 1);
          } else {
            legalMonetaryTotals.tax_exclusive_amount =
              legalMonetaryTotals.tax_exclusive_amount - diff;
    ...
    if (typeof additionalInfo === 'string') {
      if (
        additionalInfo.toLowerCase().includes('zese') ||
        additionalInfo.toLowerCase().includes('seze')
      ) {
        seze = additionalInfo;
      } else {
      ...
      for (const info of additionalInfo) {
        if (
          info.toLowerCase().includes('zese') ||
          info.toLowerCase().includes('seze')
        ) {
          seze = info;
        } else {
          notes = info;
    ...
    return {
      ...
      tax_totals: taxes,
      sendmail: this.generateDataService.sendEmail(
        customer.email,
  ...
  async generateInvoiceLines(
    ...
    if (Array.isArray(facturaDetalle)) {
      for (const detalle of facturaDetalle) {
        const unitMeasureId = await this.catalogService.getUnitMeasureIdByCode(
          detalle.unidadMedida,
        );
        const typeItemIdentificationId =
          await this.catalogService.getTypeItemIdentificationIdByCode(
            detalle.estandarCodigoProducto,
          );

        const { taxes, allowance_charges } =
          await this.generateDataService.generateTaxtotals(
            detalle.impuestosDetalles.FacturaImpuestos,
            this.catalogService,
            Number(detalle.cantidadUnidades),
          );

        const allowanceChargesGeneral: AllowanceChargeDto[] =
          this.generateAllowanceCharges(
            detalle.cargosDescuentos?.CargosDescuentos,
          );

        const allowanceCharges: AllowanceChargeDto[] = allowance_charges.concat(
          allowanceChargesGeneral,
        );
      ...
      const unitMeasureId = await this.catalogService.getUnitMeasureIdByCode(
        facturaDetalle.unidadMedida,
      );
      const typeItemIdentificationId =
        await this.catalogService.getTypeItemIdentificationIdByCode(
          facturaDetalle.estandarCodigoProducto,
        );
      const { taxes, allowance_charges } =
        await this.generateDataService.generateTaxtotals(
          facturaDetalle.impuestosDetalles.FacturaImpuestos,
          this.catalogService,
          Number(facturaDetalle.cantidadUnidades),
        );

      const allowanceChargesGeneral: AllowanceChargeDto[] =
        this.generateAllowanceCharges(
          facturaDetalle?.cargosDescuentos?.CargosDescuentos,
        );
  ...
  async generatePaymentForms(
    ...
    if (Array.isArray(mediosDePago)) {
      for (const medioDePago of mediosDePago) {
        const paymentFormId = await this.catalogService.getPaymentFormIdByCode(
          medioDePago.metodoDePago,
        );
        const paymentMethodId =
          await this.catalogService.getPaymentMethodIdByCode(
   ...
   */
  generateAllowanceCharges(
    cargosDescuento: CargosDescuentoDto | CargosDescuentoDto[],
  ): AllowanceChargeDto[] {
